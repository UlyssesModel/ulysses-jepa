"""Streaming Kafka consumer: Uhura tensor frames → ulysses-jepa narrations.
...
from ulysses_jepa.injection import EmbeddingInjectionLLM, LLMConfig
from ulysses_jepa.interfaces import AdapterConfig
...
from ulysses_jepa.pipeline import EmbeddingInjectionPipeline


logger = setup_logging("ulysses_jepa.stream")
...
class StreamConfig:
    ...
    input_topic: str
    output_topic: str
    group_id: str = "ulysses-jepa-streamer"
...
def _build_consumer(cfg: StreamConfig):
    try:
        ...
        raise ImportError(
            "confluent-kafka is required. Install with `pip install ulysses-jepa[kafka]`."
        ) from e
    return Consumer({
...
def main():
    ...
    p.add_argument("--group-id", default="ulysses-jepa-streamer")
