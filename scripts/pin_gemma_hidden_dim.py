#!/usr/bin/env python3
"""Pin the Gemma 4 31B hidden dim by querying whatever Scotty is serving.

Run this on the host where Scotty/Ollama is running. Updates
configs/adapter_default.yaml and configs/llm_gemma4.yaml in place.

Usage:
    python scripts/pin_gemma_hidden_dim.py
    python scripts/pin_gemma_hidden_dim.py --base-url http://127.0.0.1:11434
    python scripts/pin_gemma_hidden_dim.py --model gemma4:31b
    python scripts/pin_gemma_hidden_dim.py --no-write     # just print, don't update configs
...
LLM_CFG_PATH = os.path.join(REPO_ROOT, "configs", "llm_gemma4.yaml")
...
def main() -> int:
    ...
    p.add_argument("--model", default="gemma4:31b", help="Ollama model tag")
    p.add_argument("--hf-id", default="google/gemma-4-31b",
    ...
    source = None
    try:
        payload = _ollama_show(args.base_url, args.model)
        hidden = _hidden_dim_from_ollama_response(payload)
        if hidden is not None:
            source = f"ollama @ {args.base_url} ({args.model})"
    except urllib.error.URLError as e:
        print(f"[pin] Ollama query failed: {e}", file=sys.stderr)

    if hidden is None:
        hidden = _try_hf_config(args.hf_id)
        if hidden is not None:
            source = f"huggingface ({args.hf_id})"

    if hidden is None:
        print(
            "[pin] Could not determine hidden dim from any source.\n"
            "      Pass it manually:\n"
            f"        python {os.path.relpath(__file__)} --hf-id <model-id>\n"
            "      Or edit configs/adapter_default.yaml and configs/llm_gemma4.yaml directly.",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        ...
        f"[pin] llm_gemma4.yaml:     hidden_dim     {old_l} -> {hidden}"
    )
