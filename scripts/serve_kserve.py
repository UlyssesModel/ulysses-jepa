"""KServe V2 OpenInference predictor for ulysses-jepa.
...
tiberius-openshift commit f02fbc3:
    ...
    POST /v2/models/ulysses-jepa/infer
        body: {
            "inputs": [
                {
                    "name": "tensor_windows",
...
Loaded via env vars (set by the KServe manifest):
    ULYSSES_JEPA_ADAPTER_CHECKPOINT
    ULYSSES_JEPA_ADAPTER_CONFIG
    ULYSSES_JEPA_LLM_CONFIG
...
from ulysses_jepa.interfaces import AdapterConfig, KirkClient, KirkMode
from ulysses_jepa.kirk_client import (
    KirkPipelineClient,
    KirkSubprocessClient,
    StubKirkClient,
)
from ulysses_jepa.logging_config import setup_logging
from ulysses_jepa.metrics_export import metrics
from ulysses_jepa.pipeline import (
...
logger = setup_logging("ulysses_jepa.serve")
...
def _load_pipelines() -> dict[str, Any]:
    ...
    """
    adapter_cfg_path = os.environ["ULYSSES_JEPA_ADAPTER_CONFIG"]
    llm_cfg_path = os.environ["ULYSSES_JEPA_LLM_CONFIG"]
    ckpt_path = os.environ["ULYSSES_JEPA_ADAPTER_CHECKPOINT"]
...
def _build_app():
    ...
    @app.get("/v2/models/ulysses-jepa")
    async def model_info():
        return {
            "name": "ulysses-jepa",
            "versions": [_state["model_version"]],
            "platform": "ulysses_jepa",
            "inputs": [
                {
                    "name": "tensor_windows",
                    "datatype": "FP32",
    ...
    @app.post("/v2/models/ulysses-jepa/infer")
    ...
    @app.post("/v2/models/ulysses-jepa-a/infer")
    async def infer_a(request: Request):
        ...
        return await _run_pipeline(
            ...
            model_name="ulysses-jepa-a",
            max_new_tokens_kw="max_tokens",
        )

    @app.post("/v2/models/ulysses-jepa-b/infer")
