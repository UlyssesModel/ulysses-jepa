...
the heavy components (real LLM, real Kirk) mocked out. Validates:
  ...
  - /v2/models/ulysses-jepa metadata endpoint
  - /v2/models/ulysses-jepa/infer end-to-end with the sample payload
...
def test_model_metadata_shape(client):
    r = client.get("/v2/models/ulysses-jepa")
    assert r.status_code == 200
    body = r.json()
    assert body["name"] == "ulysses-jepa"
...
def test_metrics_endpoint_exposes_prometheus_text(client):
    ...
    assert body == "" or "ulysses_jepa_" in body or "# HELP" in body
...
def test_infer_with_sample_payload(client):
    ...
    r = client.post("/v2/models/ulysses-jepa/infer", json=payload)
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["model_name"] == "ulysses-jepa"
...
def test_infer_rejects_missing_inputs(client):
    r = client.post("/v2/models/ulysses-jepa/infer", json={"id": "bad", "inputs": []})
...
def test_pipeline_a_route_returns_200(client):
    r = client.post("/v2/models/ulysses-jepa-a/infer", json=_sample_payload())
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["model_name"] == "ulysses-jepa-a"
    assert "Stub narration (A)" in body["outputs"][0]["data"][0]


def test_pipeline_b_route_returns_200(client):
    r = client.post("/v2/models/ulysses-jepa-b/infer", json=_sample_payload())
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["model_name"] == "ulysses-jepa-b"
...
def test_pipeline_c_route_still_returns_200(client):
    ...
    r = client.post("/v2/models/ulysses-jepa/infer", json=_sample_payload())
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["model_name"] == "ulysses-jepa"
...
def test_each_route_calls_its_own_pipeline(client):
    ...
    r_a = client.post("/v2/models/ulysses-jepa-a/infer", json=_sample_payload())
    r_b = client.post("/v2/models/ulysses-jepa-b/infer", json=_sample_payload())
    r_c = client.post("/v2/models/ulysses-jepa/infer", json=_sample_payload())
...
def test_a_b_routes_share_input_validation_with_c(client):
    ...
    for route in (
        "/v2/models/ulysses-jepa/infer",
        "/v2/models/ulysses-jepa-a/infer",
        "/v2/models/ulysses-jepa-b/infer",
    ):
