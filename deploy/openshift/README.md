...
- **`ulysses-jepa-inference.yaml`** — KServe `InferenceService`, Red Hat AI
...
- **`ulysses-jepa-streamer.yaml`** — `Deployment` that consumes
...
```bash
podman build -t quay.io/kavara/ulysses-jepa:v0.1 -f deploy/openshift/Dockerfile .
podman push quay.io/kavara/ulysses-jepa:v0.1
```
...
oc get deployment -n kavara-ai -l app=ulysses-jepa-streamer
oc get kafkatopic -n kavara-ai | grep ulysses
oc get kafkauser -n kavara-ai | grep ulysses-jepa
```
...
```bash
ROUTE=$(oc get route ulysses-jepa-predictor -n kavara-ai -o jsonpath='{.spec.host}')
curl -X POST "https://$ROUTE/v2/models/ulysses-jepa/infer" \
...
- **Container image not built yet.** `quay.io/kavara/ulysses-jepa:v0.1` is a
