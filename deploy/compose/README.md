...
| `ulysses-jepa-predictor` | built from `deploy/openshift/Dockerfile` | KServe V2 server, stub Kirk |
...
curl -X POST http://localhost:8080/v2/models/ulysses-jepa/infer \
     -H 'Content-Type: application/json' \
     -d @samples/single_window.json

# Pipeline B — compressed text via Ollama (works without a checkpoint).
curl -X POST http://localhost:8080/v2/models/ulysses-jepa-b/infer \
     -H 'Content-Type: application/json' \
     -d @samples/single_window.json
```
...
docker compose restart ulysses-jepa-predictor
