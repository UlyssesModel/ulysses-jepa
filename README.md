...
> **Architectural narrative:** Ulysses-as-JEPA-encoder, LLM-as-predictor

## Purpose

Bridge between Kirk (Kavara's Ulysses tensor engine, two-layer pipeline:
16×16 blocks → 32×32 feature matrix → entropy) and the LLM (Gemma 4 31B via
Scotty today). Takes Kirk's outputs, projects them into the LLM's embedding
space via a small trainable MLP, and injects them directly via `inputs_embeds`.
...
```
DATA INGEST          COMPRESSION (TGE)    COMPUTE SOR          KIRK              JEPA BRIDGE              LLM
────────────         ─────────────────    ───────────          ────              ───────────              ───
polygon-ingress  →   uhura            →   tiberius-openshift → kirk-pipeline  →  ulysses-jepa         →   scotty
...
make compose-up            # start ollama + redpanda + ulysses-jepa-predictor
...
│   │   ├── ulysses-jepa-inference.yaml KServe InferenceService (V2 OpenInference)
│   │   ├── ulysses-jepa-streamer.yaml  per-cadence streaming Deployment
│   │   ├── ulysses-jepa-podmonitor.yaml UWM Prometheus auto-scrape
│   │   └── Dockerfile                  UBI9 Python 3.12, restricted-SCC
│   ├── grafana/
│   │   └── ulysses-jepa-dashboard.yaml 15-panel GrafanaDashboard CR
│   └── compose/
